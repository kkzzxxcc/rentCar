...
public class RegResDialog extends JDialog {
	JPanel jPanel;
	JLabel lResNum, lResCarNum, lResDate, lUseBeginDate, lReturnDate, lResUserId, lPrice;
	JTextField tfResNum, tfResCarNum, tfResDate, tfUseBeginDate, tfReturnDate, tfResUserId, tfPrice;
	JButton btnResReg;
	...
	String[] columnNames = { "예약번호", "예약차번호", "예약날짜", "렌터카사용시작일자", "렌터카반납일자", "예약자아이디", "가격" };
	...
	private void init() {
		...
		tfPrice = new JTextField(20); // 가격을 입력받을 텍스트 상자 추가
		...
		btnResReg.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ResVO resVO = new ResVO(resNum, resCarNum, resDate, useBeginDate, returnDate, resUserId, price,
						"예약 중"); // 상태는 "예약 중"으로 설정
				...
				tfResUserId.setText("");
				tfPrice.setText(""); // 등록 후 텍스트 상자 비우기
