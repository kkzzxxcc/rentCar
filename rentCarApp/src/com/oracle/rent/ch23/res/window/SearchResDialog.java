...
public class SearchResDialog extends JDialog {
	...
	String[] columnNames = { "예약번호", "예약차번호", "예약일자", "렌터카이용시작일자", "렌터카반납일자", "예약자아이디", "가격" };
	...
	private void loadTableData(List<ResVO> resList) {
		if (resList != null && resList.size() != 0) {
			resItems = new Object[resList.size()][8];
			for (int i = 0; i < resList.size(); i++) {
				...
				resItems[i][6] = resVO.getPrice();
				resItems[i][7] = resVO.getStatus();
			}
	...
	class ResBtnHandler implements ActionListener {
		String resNumber = null, resCarNumber = null, resDate = null, useBeginDate = null, returnDate = null, resUserId = null, price = null, status = null;
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnResSearch) {
				...
				if (selectedRow >= 0) {
					resNumber = (String) resItems[selectedRow][0];
					resCarNumber = (String) resItems[selectedRow][1];
					resDate = (String) resItems[selectedRow][2];
					useBeginDate = (String) resItems[selectedRow][3];
					returnDate = (String) resItems[selectedRow][4];
					resUserId = (String) resItems[selectedRow][5];
					double price = (double) resItems[selectedRow][6]; // 가격을 double로 가져옴
					status = (String) resItems[selectedRow][7];

					ResVO resVO = new ResVO(resNumber, resCarNumber, resDate, useBeginDate, returnDate, resUserId, price, status);
				...
				String price = (String) resItems[rowIdx][6];
				String status = (String) resItems[rowIdx][7];
				...
				if (selectedRow >= 0) {
					...
					ResVO resVO = new ResVO(resNumber, resCarNumber, resDate, useBeginDate, returnDate, resUserId, price, status);
					new ModifyResDialog(resController, resVO, "예약 수정창");
				}
				/*
